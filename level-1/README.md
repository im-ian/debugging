...
- 사용자가 이미지를 선택하고 업로드 할때 허용되지 않은 확장자의 파일이 업로드 가능하다.

#### when

- 사용자가 브라우저 등에서 파일을 업로드할 때 허용되지 않은 확장자의 파일을 업로드

#### then

1. 허용된 확장자(.jpg, .png와 같은 이미지 확장자)의 파일을 업로드하면 성공 메시지를 반환한다.
2. 허용되지 않은 확장자(.txt, .pdf)와 같은 확장자를 업로드하면 에러 메시지를 반환한다.
...
- 파일 확장자 검사 로직이 있는지 확인
- 파일 확장자 검사 로직에 허용된 확장자가 잘 등록되어 있는지 확인
- 파일 확장자 검사 로직에 '.jpg', 'jpg' 등의 검사 실수가 있는지 확인
- 파일 업로드 경로가 잘못되지 않았는지 확인
- 파일명이 올바른지 확인
...
- 파일 확장자 검사 로직이 있는지 확인
  - 파일 확장자를 검사하는 로직이 없던 문제 확인
- 파일 확장자 검사 로직에 허용된 확장자가 잘 등록되어 있는지 확인
  - 파일 확장자 검사 로직이 없어 올바른 파일 확장자가 등록되어 있지 않음
- 파일 확장자 검사 로직에 '.jpg', 'jpg' 등의 검사 실수가 있는지 확인
  - `path.extname()`로 검사 시에 '.jpg'와 같은 결과가 오는 것을 확인
  - 본인의 코드에는 'jpg'와 같이 `.` prefix가 없는 상태로 검사
  - 검사 확장자들을 `.jpg`와 같이 수정
